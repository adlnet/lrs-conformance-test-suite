...
(function (
  ...
  describe("Statement Resource Requirements (Communication 2.1)", () => {
     ...
     * An LRS's Statement API accepts POST requests
     */
    describe("An LRS's Statement Resource accepts POST requests, array payload single statement (Communication 2.1.2.s1, XAPI-00147)", function () {
      it('should persist statement using "POST"', function (done) {
        var templates = [{ statement: "{{statements.default}}" }];
        var data = helper.createFromTemplate(templates);
        data = data.statement;

        var payload = Array.from({ length: 1 }, () => Object.assign({}, data));

        payload.forEach(function (element) {
          element.object.id = element.object.id + "/" + helper.generateUUID();
          element.id = helper.generateUUID();
        });
     ...
     * An LRS's Statement API accepts POST requests
     */
    describe("An LRS's Statement Resource accepts POST requests, array payload multiple statements (Communication 2.1.2.s1, XAPI-00147)", function () {
      it('should persist statement using "POST"', function (done) {
        var templates = [{ statement: "{{statements.default}}" }];
        var data = helper.createFromTemplate(templates);
        data = data.statement;

        var payload = Array.from({ length: 2 }, () => Object.assign({}, data));
     ...
     * An LRS's Statement API upon processing a successful POST request returns code 200 OK and all Statement UUIDs within the POST
     */
    describe("An LRS's Statement Resource upon processing a successful POST request for an array payload single statement, returns code 200 OK and all Statement UUIDs within the POST **Implicit** (Communication 2.1.2.s1, XAPI-00146)", function () {
      it('should persist statement using "POST" and return array of IDs', function (done) {
        var templates = [{ statement: "{{statements.default}}" }];
        var data = helper.createFromTemplate(templates);
        data = data.statement;
        ...
        request(helper.getEndpointAndAuth())
          ...
          .end(function (err, res) {
            if (err) {
              ...
              expect(res.body)
                .to.be.an("array")
                .to.have.length.of(payload.length);
              done();
            }
          });
      });
     ...
     * An LRS's Statement API upon processing a successful POST request returns code 200 OK and all Statement UUIDs within the POST
     */
    describe("An LRS's Statement Resource upon processing a successful POST request for an array payload multiple statement, returns code 200 OK and all Statement UUIDs within the POST **Implicit** (Communication 2.1.2.s1, XAPI-00146)", function () {
      it('should persist statement using "POST" and return array of IDs', function (done) {
        var templates = [{ statement: "{{statements.default}}" }];
        var data = helper.createFromTemplate(templates);
        data = data.statement;
 ...
 */
    describe('An LRS\'s Statement Resource upon processing a successful GET request with neither a "statementId" nor a "voidedStatementId" parameter, returns code 200 OK and a StatementResult Object.  (Communication 2.1.3.s1, XAPI-00154)', function () {
      ...
      before("persist substatement", function (done) {
        ...
        substatement.object.context.contextActivities.category.id =
        ...
        request(helper.getEndpointAndAuth())
          ...
          .end(function (err, res) {
            if (err) {
              ...
              var boundary = multipartParser.getBoundary(
      ...
      it("should apply this data to choose the matching language in the response", function (done) {
        ...
        request(helper.getEndpointAndAuth())
          ...
          .expect(200, function (err, res) {
            ...
            expect(
              statement.context.contextActivities.category[0].definition
                .description
            ).not.to.have.property("en-US");
            expect(
              statement.context.contextActivities.category[0].definition.name
            ).not.to.have.property("en-US");
            done(err);
          });
      });
    ...
    describe('An LRS\'s Statement Resource can process a GET request with "format" as a parameter  (**Implicit**, Communication 2.1.3.s1.table1.row12)', function () {
      ...
      it('should process using GET with "format" canonical (XAPI-00169)', function (done) {
        ...
        canonicalSubActivity.definition.name["en-GB"] =
          activity.definition.name["en-GB"];
        canonicalSubActivity.definition.description = {};
        canonicalSubActivity.definition.description["en-GB"] =
        ...
        canonicalSubActivity.definition.interactionType =
          activity.definition.interactionType;
        canonicalSubActivity.definition.correctResponsesPattern =
          activity.definition.correctResponsesPattern;
        canonicalSubActivity.definition.extensions =
    ...
    describe('An LRS\'s Statement Resource can process a GET request with "attachments" as a parameter  (**Implicit**, Communication 2.1.3.s1.table1.row13, XAPI-00167)', function () {
      ...
      before("set up statement with two attachments for test", function (done) {
        var header = {
          "Content-Type":
            "multipart/mixed; boundary=-------314159265358979323846",
        };
        var templates = [
        ...
        txtAtt1 = fs.readFileSync(
          "test/v1_0_3/templates/attachments/simple_text1.txt"
        );
        var t1stats = fs.statSync(
          "test/v1_0_3/templates/attachments/simple_text1.txt"
        );
        t1attSize = t1stats.size;
        t1attHash = crypto.createHash("SHA256").update(txtAtt1).digest("hex");

        txtAtt2 = fs.readFileSync(
          "test/v1_0_3/templates/attachments/simple_text2.txt"
        );
        var t2stats = fs.statSync(
          "test/v1_0_3/templates/attachments/simple_text2.txt"
        );
        ...
        msg +=
          "X-Experience-API-Hash: " + data.attachments[0].sha2 + crlf + crlf;
        msg += txtAtt1 + crlf;
        msg += dashes + boundary + crlf;
        msg += "Content-Type: text/plain" + crlf;
        msg += "Content-Transfer-Encoding: binary" + crlf;
        msg +=
          "X-Experience-API-Hash: " + data.attachments[1].sha2 + crlf + crlf;
        msg += txtAtt2 + crlf;
 ...
 */
        ...
        request(helper.getEndpointAndAuth())
          ...
          .end((err, res) => {
            if (err) {
              ...
              var boundary = ContentType.split(";")[1].replace(
                " boundary=",
                ""
              );
              ...
              for (var i in body) {
                idx = Math.max(
                  body[i].indexOf("here is a simple attachment"),
                  idx
                );
              }
    ...
    describe("An LRS's Statement Resource, upon processing a successful GET request wishing to return a Voided Statement still returns Statements which target it (Communication 2.1.4.s1.b2, XAPI-00162)", function () {
      ...
      before("persist voided statement", function (done) {
        ...
        sinceVoidingTime = new Date(
          Date.now() - helper.getTimeMargin() - 4000
        ).toISOString();
            ...
            if (err) {
              ...
              try {
                ...
                if (e.message.length > 400) {
                  e.message =
                    "expected results to have property 'statements' containing " +
                    voidingId;
                }
                done(e);
              }
        ...
        request(helper.getEndpointAndAuth())
          ...
          .end(function (err, res) {
            if (err) {
              ...
              expect(results.statements[0])
                .to.have.property("id")
                .to.equal(statementRefId);
              done();
            }
          });
      });
    ...
    describe('The Statements within the "statements" property will correspond to the filtering criterion sent in with the GET request (Communication 2.1.3.s1, XAPI-00164)', function () {
      ...
      before("persist substatement", function (done) {
        ...
        substatement.actor.mbox =
          "mailto:" + helper.generateUUID() + "@adlnet.gov";

        substatement.object.verb.id += helper.generateUUID();
        substatement.object.actor.mbox =
          "mailto:" + helper.generateUUID() + "@adlnet.gov";
            ...
            if (err) {
              ...
              expect(result)
                ...
                .to.satisfy(function (statements) {
                  for (var i in statements) {
                    if (
                      new Date(statements[i].stored) <
                      new Date("2012-06-01T19:09:13.245Z")
                    )
                      return false;
                  }
                  return true;
              ...
              request(helper.getEndpointAndAuth())
                ...
                .end(function (err, res) {
                  if (err) {
                    ...
                    expect(boundary).to.be.ok;
                    var parsed = multipartParser.parseMultipart(
                      boundary,
                      res.body
                    );
                    expect(parsed).to.be.ok;
